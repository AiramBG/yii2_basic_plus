...
use app\src\common\components\i18n\events\LanguageChangedEvent;
use Yii;
...
/**
 * This class extends Yii's UrlManager and adds features:
 *  - It detects the language from the URL or from browser settings transparently
 *  - It persists the language in the user session and optionally in a cookie
 *  - It adds a language parameter to any created URL
 */
class UrlManager extends BaseUrlManager
{
     ...
     * Leave empty to be filled from DB.
     */
    public $languages = [];
    ...
    /** @var bool whether to translate the requested url to reach a controller/action.
     *
     * If this is `true` (default value), then a request `es/quienes_somos` will be
     * redirected to landing/about.
     *
     * If this is `false`, then the request must be `/es/some/page`.
     */
    public $translateUrl = false;
    ...
    /**
     * @var bool Only works if default Controller Action is 'index';
     */
    public $hideActionIndex = true;
    ...
    {
        if (!$this->languages) {
            $this->languages = (new TranslationManager())->getLangCodes();
        }
    ...
    public function createUrl($params)
    {
        ...
        if ($this->enableLocaleUrls && $this->languages) {
            ...
            if ($this->translateUrl) {
                $gutter = explode('?', $url);
                $gutter[0] = Yii::r($gutter[0], [], $language);
                $url = implode('?', $gutter);
            }
            ...
            if (
                ...
                //Remove unnecessary '/index' on pretty urls
                if ($this->hideActionIndex && strpos($url, '/index') !== false && strpos($url, '/index.php') === false) {
    ...
    protected function processLocaleUrl($normalized)
    {
        ...
        if (preg_match("#^($pattern)\b(/?)#i", $pathInfo, $m)) {
            ...
            if ($this->translateUrl) {
                $pathInfo = Yii::r('/'. $pathInfo, [], Yii::$app->language);
            }
